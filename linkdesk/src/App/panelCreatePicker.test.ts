...
describe("showPanelCreatePicker（E5.8#32）", () => {
  ...
  it("数据源两级展平——容器序 × 全量视图（含已隐藏视图），非仅活跃", () => {
    ...
    expect(items.map((it) => it.viewId)).toEqual(["demo-view-a", "demo-view-b", "demo-view-c"]);
    // 隐藏视图也进列表（选中可恢复可见）
    expect(items.find((it) => it.viewId === "demo-view-b")?.visible).toBe(false);
  ...
  it("serialize——已激活项 checked:true，其余 false；隐藏视图 category 标「已隐藏」", () => {
    ...
    expect(probs.label).toBe("Alpha");
    expect(probs.category).toBe("demo-plugin-a · Demo A");
  ...
  it("onSelect——激活视图切换；隐藏视图选中自动恢复可见", () => {
    ...
    showPanelCreatePicker(setActive, { current: "demo-view-a" });
    const st = captureShow();
    ...
    expect(setActive).toHaveBeenCalledWith("demo-view-a");

    // 选中隐藏视图 → 恢复可见 + 切换激活
    onSelect(items[1]);
    expect(setVisibleMock).toHaveBeenCalledWith("demo-panel-a", "demo-view-b", true);
    expect(setActive).toHaveBeenCalledWith("demo-view-b");
  });
  ...
  it("getKey = viewId——全局唯一键（池动作重解析 key）", () => {
    ...
    expect(getKey(items[1])).toBe("demo-view-b");
  });
});
