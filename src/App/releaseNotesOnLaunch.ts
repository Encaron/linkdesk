 ...
 *
 * 2. **首启自动弹**（`_run`）——`app.getVersion() !== lastSeenVersion` ⇒ 自动打开标签页一次、
 *    并**带着当前版本号**发起取数（理由见下方 ③ 与那个调用点的 🔴）。
 *
 * ## 三处容易做错的地方（都已在实现里钉住）
 *
 ...
 *
 * ③ **取数必须点名「当前运行的这一版」**（`#57.13h` 收尾时订正，2026-09-13；原实现点的是空）。
 *    这一格空着 ⇒ 主进程「所请求的那一版不在缓存里 ⇒ 视为失效」那条判据短路 ⇒ 升级前拉的缓存
 *    照旧命中 ⇒ 首启看到的是**上一版**的说明、横幅还把它当新版宣告。详见调用点的 🔴。
 *
 * ## 与「关闭开关」的关系
 *
...
async function _run(): Promise<void> {
  ...
  // 🔴 **版本号必须带下去，不是可选项**（`#57.13h` 收尾时订正，2026-09-13）。
  //    主进程那条失效判据「**所请求的那一版不在缓存里 ⇒ 视为失效**」（05 §2.4 第二条，设计原文
  //    写的就是「救刚更新完的首次启动」这个场景）**只在请求带了版本号时才成立**——请求里那一格空着，
  //    判据就短路，缓存（升级前拉的、根本没有你刚装上的那一版）照旧命中，首启横幅就会把**上一版**
  //    当新版宣告。带上它两条一起好：缓存里没有你这一版 ⇒ 真出网重拉 ⇒ 页面上就是你刚装上的那版，
  //    横幅才配叫「新版本」。
  await openReleaseNotesTab({ banner: true, version });
