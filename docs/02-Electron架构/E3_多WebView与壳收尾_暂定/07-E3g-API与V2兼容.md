...
| # | 任务 | 行数 | 独立验证 |
|:--:|------|:--:|------|
| — | 动态 StatusBarItem——运行时创建 + dispose 自动清理 | ~50 | 插件 mount → 状态栏出现 → unmount → 消失 |
| — | contributes.icons——共享图标注册 + 引用 | ~30 | 插件 A 贡献图标 → 插件 B 的 icon 字段引用 → 渲染 |
| — | ☰ 完整版——快捷键提示 + 禁用态灰显 + 插件菜单组 | ~100 | ☰ → 右侧灰字快捷键 → when 不满足项灰色 → 插件菜单多一列 |
| — | V2 配置导入——映射表 + 文件选择 + 逐 key 迁移 | ~60 | 选 V2 prefs.json → 波特率/编码/主题导入 |
| **合计** | | **~240 行** | |
