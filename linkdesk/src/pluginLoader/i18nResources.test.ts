...
import { registerPluginLanguageBundle, unregisterPluginLanguageBundles } from "./i18nResources";
...
afterEach(() => {
  ...
  for (const pid of TEST_PLUGINS) {
    unregisterPluginLanguageBundles(pid);
  }
});
...
describe("i18nResources——重装契约：卸载时 translation 命名空间按剩余插件重建", () => {
  ...
  it("卸载重建——被卸插件键消失，其余插件同顶层键的子键幸存（碰撞正确）", () => {
    ...
    unregisterPluginLanguageBundles("plugin-a");
  ...
  it("注册序保持——重建后剩余插件仍按注册序合并且各自键完整", () => {
    ...
    unregisterPluginLanguageBundles("plugin-b");
    const bundle = i18n.getResourceBundle("zh", "translation") as Record<string, unknown>;
  ...
  it("未知插件卸载不崩 + 重装同 pluginId 干净重入", () => {
    expect(() => unregisterPluginLanguageBundles("ghost")).not.toThrow();
    registerPluginLanguageBundle("zh", { "x.y": 1 }, "plugin-c");
    unregisterPluginLanguageBundles("plugin-c");
