...
| Shared component classes + host container classes | **the whole `ldk-` namespace**—today: `ldk-badge`, `ldk-button`, `ldk-combobox`, `ldk-mdv`, `ldk-selectbox`, `ldk-sle`, `ldk-slider`, `ldk-toggle`, `ldk-hint-card` (2026-09-19, hover hint card) (shared components), plus the host's own container classes such as `ldk-titlebar`, `ldk-input`, `ldk-setting-group` | Class names of the badge / button / combobox / Markdown container / select / string-list editor / slider / toggle components in `@linkdesk/ui`. **You don't have to memorize them**—remember one thing: **anything starting with `ldk-` belongs to the host itself (shared components + host UI containers), so don't put it on your own elements** (the one exception is `.ldk-input` on the row above: that one is **for you**) |
...
- **What changed**: §12.1 covered the eight base names of `@linkdesk/ui` (`badge` / `button` / `combobox` / `mdv` / `selectbox` / `sle` / `slider` / `toggle`) at **0.2.0**. The remaining components' class names—plus one `@keyframes` name—now carry the prefix too (**since the 2026-09-16 round**; for how to read the version line see [19-Component Cheatsheet §2.1](19-component-cheatsheet.md)):
...
- 🔴 **When the host renames a shared name or restyles a component, there is no code and no CSS for you to change** — components and styles are supplied by the shell pool (L9 centralized supply, see [19-Component Cheatsheet §2](19-component-cheatsheet.md)): the same implementation follows the shell upgrade, so your UI stays consistent. The one thing to maintain is that your `@linkdesk/ui` dependency **stays on the current version line** (it decides what your local dev and types resolve to).
- ⛔ **Component styles never come from your source either**: a line like `import "@linkdesk/ui/index.css"` (or its dist / `@import` form) is failed by the SDK lint leg `linkdesk/no-ui-css-import`, with **no exemption exit** — baking the styles into your plugin means "the shell restyles once, the whole ecosystem follows" stops being true for you.
- **When you must change it**: when the shell you support moves to a version that contains this rename (how to read the version line: [19-Component Cheatsheet §2.1](19-component-cheatsheet.md) — ui version = shell version, one line). After upgrading, **grep your own CSS for the old families** in the table above: an official plugin (`settings`) had exactly one such spot and it was fixed in the same release batch.
