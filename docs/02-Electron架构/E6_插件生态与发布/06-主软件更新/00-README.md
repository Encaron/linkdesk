...
5. **更新交互全走通知面——不新造 UI 表面**（复用壳 `pushToast`/`updateToast`/actions/isCloseAffordance，先例 03-插件市场「事件型提示一律走通知面」）。⚠️ **2026-09-11 用词校正**：当时把载体写成「toast」= **右下窄小卡 `ToastHost`**，该载体已被 **E6#72a 整删**；现行唯一通知面 = **状态栏铃铛宽面板**（E6#72 归一 + 03-插件市场/[17-通知面归一任务档案](../03-插件市场/17-通知面归一任务档案.md)/[18-通知系统全账与设计定案](../03-插件市场/18-通知系统全账与设计定案.md)）⇒ **本条决定零变更**（仍「复用壳通知机制 + 零新 UI 表面」，入口 API 仍是 `pushToast`/`updateToast`/`actions`/`isCloseAffordance`），变的只是**它渲染在哪个面**。本文件其余处 2026-08-30 行文中的「toast」同此读作「通知面条目」。
...
8. **两条路完整流程（用户 2026-08-30 亲述）**——路 A（被动）与路 B（主动）触发方式不同，之后完全汇合；无更新时入口恒显、通知面即查即答。总览见 §一·五 + mockup 02 Frame 0。

## 一·五、两条路完整流程（2026-08-30 用户亲述）

> ⚠️ **2026-09-11 用词校正**：下方流程图写于 2026-08-30，「toast」= 当时的右下窄小卡载体。**该载体已由 E6#72a 整删**，现行载体 = **状态栏铃铛宽面板**（`.status-bar-notif-panel`，条目**恒全显**、带 4px 真进度条）。**流程本身一字未变**，只把载体名读作「通知面条目」。**图与帧已按现行载体重画** → [mockups/02-更新通知与关于.html](mockups/02-更新通知与关于.html)。

```
路 A · 被动（有新更新自动出现）                路 B · 主动（帮助菜单）
有新版本，即便不点检查更新              帮助菜单「检查更新…」（恒显）
   ↓ auto 后台检查到 available               ↓ 点击
TitleBar「下载更新」按钮自动出现（主题按钮色）   无更新 → 条目「当前已是最新版本」
   ↓ 点击（openUpdateFlow）                   ↓ 有更新
从按钮启动下载 ═══════════════►         [发现更新条目] → 点「立即更新」
                                             │ 从条目启动下载
                                             ▼
               ┌── 两条路汇合（同一流程）────────────────────┐
               │ 下载中 → 条目「正在下载更新 42%」+ 4px 真进度条  │
               │          + TitleBar 按钮变「更新中」            │
               │ 下载完成 → 条目「更新已下载，重启后生效」        │
               │            [稍后][重启并更新] + 按钮变「重新启动」│
...
- **触发方式两条路，流程一条**：路 A 用户不做任何事（auto 后台检查到 → 按钮 + 条目一起来）；路 B 用户主动查（恒显入口，最新就答「已是最新」，有更新就进同一流程）。
- **无更新**：条目「当前已是最新版本」即查即答（手机 App 同款）——「检查更新」入口恒显的底气。
- 对应帧：mockup 02 Frame 0（总览图）/ Frame 1（发现）/ Frame 2（下载中）/ Frame 3（完成）/ Frame 4（手动无更新）/ **Frame 5（失败 + [重试]）** / **Frame 6（关于页）**。
...
- CDP 实机：菜单项恒显、TitleBar 按钮按状态门控显隐（全文字 + 主题按钮色）、通知面全流程（铃铛面板：发现 / 进度 / 完成 / 失败+重试 / 手动无更新）、发行说明标签页、重启安装。
- 🔴 **逐帧对照 mockup**（01 帮助/齿轮/按钮 · 02 通知面 Frame 0-6 · 03 发行说明 Frame 1-3）——**不是「大致像」**。
- 🔴 **四份文档的验收点表全过**：[01 §四](01-更新机制设计.md)（14 点，含陈旧 exe 清理 / Downloading 不复活 / 六类归因 / 多窗口）、[03 §六](03-菜单与入口设计.md)（8 点）、[04 §七](04-更新通知与交互.md)（11 点）、[05 §四](05-发行说明.md)（8 点）——四张表**逐条**打勾，**允许留白但不允许不查**。
- 🔴 **资产名/版本号断言**（[05-文档与发布/02-发布流水线.md](../05-文档与发布/02-发布流水线.md) §〇 / E6#42e）：安装包文件名逐字符 = `linkdesk-setup-{version}.exe`；CHANGELOG 含 `## v{version}` 段。
...
| [04-更新通知与交互.md](04-更新通知与交互.md) | **全走唯一通知面**（铃铛宽面板：发现/进度/完成/失败/手动结果；⚠️ 2026-09-10 载体校正，见该档头部） | ✅ |
| [05-发行说明.md](05-发行说明.md) | 标签页 vs 浏览器 / 数据源 / 自动弹 / 可关 | ✅ |
| [06-关于标签页.md](06-关于标签页.md) | 字段 / 载体方案 A 标签页（已拍板）/ 数据来源 | ✅ |
| [07-数据流通格式.md](07-数据流通格式.md) | IPC 通道 / 消息类型 / TS 类型 / 事件 payload / context key | ✅ |
| [08-调度归属与失败重试.md](08-调度归属与失败重试.md) | 调度者身份（壳唯一实例的源码实证 + 将来多壳的判据）/ 下载失败重试（现况 + 待回答项 + 判据草案）/ 失败记账里没有「谁发起的」（后台静默判据缺数据路径） | 🟡 §一 已结案（证据结案不写守卫）；§二/§三 决定已定、实现未开工（账挂 #57.9g / #57.12f） |
| [01-帮助菜单与更新按钮形态.html](mockups/01-帮助菜单与更新按钮形态.html) | **Frame 1** 帮助菜单 / **Frame 2** 齿轮菜单 / **Frame 3** TitleBar 更新按钮（可点切换四态：不出现 / 下载更新 / 更新中 / 重新启动，**全文字 + 主题按钮色**，实机对齐） | ✅ |
| [02-更新通知与关于.html](mockups/02-更新通知与关于.html) | **Frame 0** 两条路完整流程总览 / **1** 发现 / **2** 下载进度（4px 真条）/ **3** 下载完成重启 / **4** 手动检查无更新 / **5** 失败 + `[重试]` / **6** 关于页——全在**铃铛面板条目**载体上（实机对齐 `StatusBarZone.css`）。⚠️ **2026-09-11 重画**：旧版（`02-更新通知toast与关于.html`）画的是已死的右下小卡 `ToastHost` + 默认折叠 + 无进度条，已作废删除 | ✅ |
| [03-发行说明标签页.html](mockups/03-发行说明标签页.html) | 发行说明壳内标签页（对齐插件详情页 01 竞标 A 骨架：头部版本区 + 主体 = **左窄栏版本历史 220px（配角）+ 右主区更新内容（主角）**；**内容不占声明式侧栏**，外层画出完整软件框架示意——图标栏/应用侧栏/状态栏）。**Frame 1** 主态（有内容）/ **Frame 2** 断网空态（无缓存）/ **Frame 3** 加载中骨架屏（⚠️ 2026-09-11 补画——原稿只有前两态，中间那段「正在拉」没有图） | ✅ |
