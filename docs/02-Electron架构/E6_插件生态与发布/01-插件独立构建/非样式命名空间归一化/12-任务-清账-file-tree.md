...
### 1.1 逐项清账面（**75 处**，复跑：`node scratch/audit-plugin-scope.mjs`）
...
2. 🔴 **必须回报三组读数**：① 改名前/后 `node scratch/audit-plugin-scope.mjs` 的**本仓处数**（期望 75 → **0**）；② 形状证明与残留判据读数；③ §2.6 三条实证读数。
...
# 2) 自量本仓（面清单只用来对账）
node scratch/audit-plugin-scope.mjs | sed -n '/── file-tree/,/^──/p'
