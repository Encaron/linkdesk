...
describe("groupSettingsKeys — E5.8#78 组内二级标题归桶", () => {
  it("有 group 的 key 归到对应桶，无 group 的 key 归平铺桶——顺序保持", () => {
    const buckets = groupSettingsKeys(
      ["app.theme", "app.themeColor", "app.surfaceRadius", "app.mixMode"],
      mkProps({ "app.theme": "整体配方", "app.themeColor": "配色", "app.mixMode": "域混搭" })
    );
    expect(buckets).toEqual([
      { group: "整体配方", keys: ["app.theme"] },
      { group: "配色", keys: ["app.themeColor"] },
      { group: "", keys: ["app.surfaceRadius"] },
      { group: "域混搭", keys: ["app.mixMode"] },
    ]);
  });
  ...
  it("主题组 5 分节全量归位——每个分节桶 key 正确", () => {
    const buckets = groupSettingsKeys(
      ...
      (k) => {
        if (k === "app.theme") return "整体配方";
        if (k === "app.themeColor") return "配色";
        if (k === "app.accentMode" || k === "app.accentColor") return "强调色";
        if (k.startsWith("app.mix")) return "域混搭";
        return "外观覆盖"; // 其余全归外观覆盖（appearanceMode → zoneBackgroundImage）
      }
    );
    expect(buckets.map((b) => b.group)).toEqual([
      "整体配方", "配色", "强调色", "外观覆盖", "域混搭",
    ]);
    expect(buckets[3].keys).toEqual([
      "app.appearanceMode", "app.surfaceRadius", "app.glassBlur", "app.glassOpacity",
      "app.glassTint", "app.backgroundImage", "app.fontFamily", "app.zoneRadius",
      "app.zoneRadiusScale", "app.zoneBackgroundImage",
    ]);
    expect(buckets[4].keys).toEqual([
      "app.mixMode", "app.mixFont", "app.mixRadius", "app.mixGlass",
