...
> 2026-09-09 · E6#70e 文档化（70a-d 四形态实现全闭后补作者契约）。**写给第三方插件作者。**
> 这份文档回答一个问题：**你的 README 在 LinkDesk 软件里会被插件详情页展示——里面想放图 / 动图 / 演示视频，怎么写才对、有什么规矩。** 对标 VS Code 更新页 / 扩展 README 的开放展示画布。
> 性质 = **接口契约**——只定义「作者怎么写得对 + 会显示成什么样」，不写实现代码（实现面 = 壳共享 MarkdownView + 市场 DetailView，见 [详情页说明展示任务档案](../02-Electron架构/E6_插件生态与发布/03-插件市场/15-详情页说明展示任务档案.md)）。作者不需要知道 App 怎么渲染，把下面几条写对即可。
...
- 市场图标双字段 `marketIcon` / 封面：`../02-Electron架构/E6_插件生态与发布/03-插件市场/06-图标.md`
- 实现档案（何时显示、为什么不显示）：`../02-Electron架构/E6_插件生态与发布/03-插件市场/15-详情页说明展示任务档案.md`
