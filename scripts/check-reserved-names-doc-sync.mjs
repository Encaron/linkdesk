#!/usr/bin/env node
/**
 * 作者面文档门禁 ④ —— **保留名清单的单一真相源**（E6#109i）。
 *
 * 出处（唯一真源，本文不重述判据）：`docs/02-Electron架构/E6_插件生态与发布/01-插件独立构建/样式命名空间归一化/11-任务-保留名清单单一真相源.md`
 *   ＋ 总档案 `00-整理档案.md`（件 3）。
 *
 * ── 判据一句话 ──
 *   机器侧真源 = `packages/plugin-sdk/schemas/reserved-class-names.json`；作者读的那份 = §12 的「保留名」
 *   表（`docs/03-插件制造/05-插件UI写法规约.md` 与 `docs/03-plugin-authoring/05-ui-conventions.md`
 *   **两份手抄**）。本门禁把这三份**双向**钉在一起：
 ...
 *   · 只扫该节里**表头含「保留名」/「Reserved names」那张表的那一列**。列位置按表头找、不写死第几列；
 ...
 *   node scripts/check-reserved-names-doc-sync.mjs --self-test  # 正控绿 ＋ 六个负控红
...
/** 读登记表 → { bare: Map<name, why> }（裸名 = 登记表管的那种；带前缀的自带命名空间、不登记） */
 ...
 * @param {{docs:{label:string,text:string}[], registeredBare:Map<string,string>, ldkTokens:Set<string>}} input
 * @returns {{kind:string,msg:string}[]}
 */
export function checkSync({ docs, registeredBare, ldkTokens }) {
  const violations = [];
...
const fixture = ({ hostCell = "`.input`", registry = ["input"], ldk = "`ldk-badge`, `ldk-titlebar`" }) => ({
  docs: [
    {
      label: "zh-fixture",
...
const violations = checkSync({
  docs,
  registeredBare: loadRegistry().bare,
...
if (violations.length === 0) {
  ...
  console.log(
    `✅ [reserved-names-doc] §12 保留名表与登记表双向一致（裸名 ${bare.length} 个：${bare.map((n) => "." + n).join(", ")}；` +
      `ldk- 名 ${ldk.length} 个）；中英两棵树名字集合相等。`
  );
  process.exit(0);
}
