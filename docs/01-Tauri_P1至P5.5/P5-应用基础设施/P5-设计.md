...
> 
> **深度分析：** 类型系统去硬编码——来龙去脉 / Phase 3→4→5 的历史 / "单线程→多线程"比喻 / VS Code EditorInput 对照 / 利弊权衡总表——详见本文 §11。
...
## 十二、归一化设计决策——ConfigurationApplier 模式（原独立文档摘录）

Phase 5 引入 `ConfigurationApplier` 模式解决"配置定义和配置效果分离"的问题。

**核心原则：** 配置的"定义"和"效果"写在一起。`plugin.json` 声明 `contributes.configuration`（定义），`onApply` 回调注册在 ConfigurationRegistry（效果）。加载器读到声明 → 自动注册配置定义 + 调用 onApply 应用当前值。

**归一化收益：** 新增一个配置项不需要改任何 React 组件——只需在 plugin.json 加一行 + 在 onApply 加一个 case。Settings Editor 自动渲染对应控件（Toggle/Select/Input）。

---

## 十三、运行时动态加载（原独立文档摘录）

Phase 5h 实现插件的运行时动态加载——安装/卸载/重装无需重启软件。

**双路径：** 出厂预装插件通过 Vite `import.meta.glob` 构建时打包；运行时安装的插件通过 Tauri fs 读取 → Blob URL → `import()`。

**PluginLifecycle 归一化：** 安装/卸载/启用/禁用的生命周期事件统一走 `PluginLifecycle` Emitter。loader.ts 中的 `loadPlugin`/`loadPluginRuntime` 两条路径对等处理 `mode`/`resources` 字段。

**关键修复：** `plugin://` 硬编码路径改为 Vite `/@fs/` 端点 + Rust `resolve_plugin_path`，修复所有非 glob 插件的加载失败。
