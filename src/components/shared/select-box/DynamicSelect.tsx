...
function DynamicSelect({ value, onChange, optionsFrom, domain, disabled, placeholder, title }: DynamicSelectProps) {
  ...
  // E5.8#90：外观主开关 app.appearanceMode 决定走 sources（custom）还是 colorways（跟随主题）。
  const dualSemantics = optionsFrom === "theme.colorways" && domain === "colors";
  const [mode, setMode] = useState<string | undefined>(undefined);

  // 双语义才订阅 app.appearanceMode——只监听关心 key（E5.8 铁律）；切外观模式重渲染走新来源
  useEffect(() => {
    if (!dualSemantics) return;
    let cancelled = false;
    void window.linkdesk?.configuration?.get<string>("app.appearanceMode").then((v) => {
      if (!cancelled) setMode(v);
    });
    const off = window.linkdesk?.configuration?.onChange?.("app.appearanceMode", (v: unknown) => setMode(v as string));
    return () => { cancelled = true; off?.(); };
  }, [dualSemantics]);

  const isCustomMode = dualSemantics && mode === "custom";
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (optionsFrom === "theme.sources" || isCustomMode) {
        if (domain === "colors") {
  ...
  }, [optionsFrom, domain, t, isCustomMode]);
  ...
  if (dualSemantics && !isCustomMode && options.length === 1) return null;
