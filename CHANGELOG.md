...
- **feat:E6#47 一个工程一个窗口——像 VS Code 那样，右键/命令行带文件夹就以新窗口打开它**（第 5.3 轮｜新增多窗口能力 ⇒ 0.x 向后兼容走 patch 位）
  - **怎么用**：对文件夹右键选「Open with LinkDesk」、或在终端敲 `linkdesk D:\某工程`，都会**新开一个独立窗口**；A、B 两个工程各开一窗、互不干扰（各自的文件树、标签页、布局、展开状态都分开存）。
  - **关掉全部再双击图标**：会**恢复最后一次用的那个工程**（只恢复它一个，跟 VS Code 的「最后活跃窗口」一致）；工程文件夹已经删了的话，就开一个空工作区，不会报错。
- **feat:E6#46 命令行/文件关联能打开文件了——`linkdesk 文件.txt`、双击关联文件都进编辑器**（第 5.3 轮｜新增 intake 链路 ⇒ patch 位）
  - **怎么用**：命令行带文件参数 → 在窗口里用编辑器打开；同一文件再开一次不会重复开标签，而是**跳到已有标签**；扩展名没有对应插件的文件走编辑器兜底。
  - **一个细节**：如果这次启动本身就是来开文件的，**「发行说明」不再抢焦点弹出来盖住它**（只预热不弹）——用户点名的文件优先。
  - **真机打磨（2026-09-13 装机实测发现并修）**：① 命令行启动时**不会再把 LinkDesk.exe 自己**当成文件开出一个标签（`second-instance` 的 argv 首项是 exe 自身，原先没跳过）；② **有标签页要恢复时文件也照常打开**——原先这个次序是反的，打开的文件标签会被「恢复上次标签页」整体覆盖掉（空环境测不出来，装了正式 profile 才现形）。
- **feat:E6#45 Shell 集成——安装时可选「附加任务」，右键菜单带图标**（第 5.3 轮｜安装器新增勾选页与注册表写入 ⇒ patch 位）
  - **装的时候**会多出一页「选择附加任务」（照 VS Code 的样子）：文件右键菜单（默认不勾）/ 文件夹右键菜单（默认不勾）/ 注册为受支持文件类型的编辑器（默认勾）/ 添加到 PATH（默认勾）。
  - **右键菜单**出现在资源管理器的「显示更多选项」层，带 LinkDesk 图标（Win11 新式顶层菜单本轮不做，已立案）。
  - **软件里也能改**：设置 →「通用」组多了三个开关（复用现有胶囊开关），勾一下就**立即生效、不用重装**——这一点比 VS Code 强（它只能重跑安装器）；注册表是唯一真相源，安装时勾了什么，软件里就显示为开。
  - **卸载**会把右键菜单、文件关联（只删自己写的部分，不动 `.txt` 这类扩展名键本身）、PATH 里自己加的那段清理干净；用户自己改过 PATH 时不误删。
  - **静默升级不会复活**你已经关掉的那两项（以注册表现状反推，不硬编码默认值）。

## v0.1.60（2026-09-13）

- **feat:E6#57.10e 许可证定案 MIT——仓库补上 LICENSE 正本，帮助菜单多一项「查看许可证」**（第 5.4 轮｜一份 LICENSE 文件 ＋ 一条菜单命令 ⇒ 纯新增，0.x 向后兼容走 patch 位）
  - **为什么是现在**：仓库一直没有 `LICENSE` 文件，但发到 npm 的四个作者包早就写着 MIT——**对外生效的声明没有正本支撑**。2026-09-13 用户拍板选 MIT，本版把正本补上。
  - **怎么看到它**：帮助菜单末尾多一项**「查看许可证」**（独占一组，与上面两项之间有一条分隔线），点它在**系统浏览器**打开 GitHub 上的许可全文——与发行说明页的「在 GitHub 查看全部」是同一条外链路。⚠️ 生效前提 = LICENSE 推到远端（当前攒本地未推）。
  - **说明**：`package.json` 同时补上 `"license": "MIT"` 字段；npm 四包本就声明 MIT，无需重发。
- **fix:E6#37f 插件加载入口加了道「坏 id」守卫——来路不明的插件标识再也流不进加载器**（第 5.0 轮立案｜一段边界判断 ⇒ 纯防御，0.x 向后兼容走 patch 位）
  - **它防的是什么**：历史日志里出现过 `[object Object]` 被当成插件 id 拼进加载地址（六次）。现在加载器第一件事就是验 id 是不是非空字符串——不是就当场拒绝加载，并把**完整内容**打进诊断日志（原先的写法会把对象吃成 `[object Object]`，线索全无）。正常使用无感知。
- **fix:E6#57.7b 点「重启并更新」之前先验货——损坏的更新包装不上也不再让您干等**（第 5.4 轮｜安装腿一段校验 ⇒ 纯防御，0.x 向后兼容走 patch 位）
  - **它防的是什么**：实测发现，如果盘上那份已下载的更新包事后损坏或被替换，点「重启并更新」后软件会关掉、坏包装不上、软件也不回来——**整个过程一点提示都没有**。现在退出安装前先核对包的指纹，对不上就当场删掉坏包、给您报一句「校验不通过，请重新下载」，软件照常开着。

## v0.1.59（2026-09-13）

- **fix:E6#57.13 刚更新完的那一次启动，发行说明不再把「上一版」当成「新版本」念给您听**（第 5.4 轮｜取数点名当前版本号 ＋ 横幅多一道闸 ⇒ 纯修复，0.x 向后兼容走 patch 位）
  - **它防的是什么**：装上新版本后的第一次启动会自动打开发行说明，顶上挂一句「检测到新版本 x.y.z」。可这一页有一份**本地存档**（省流量用的，24 小时内不再联网）。**刚升级完的那一刻，存档多半是升级前拉的**——里面根本没有您刚装上的那一版。于是那句话会念出**您刚替换掉的那个版本号**：明明是升级，横幅却说「检测到新版本 0.1.54」。**这句话从真话变成假话，只需要一次升级**，而且天天开软件的人每次升级都会撞上。
  - **病根不是缓存，是取数时那一格空着**：设计文档里本来就写着一条判据——「**点名要的那一版不在存档里 ⇒ 存档作废、重新联网拉**」，并且**明写它救的就是「刚更新完的首次启动」**这个场景。但自动弹那条路发起取数时**没有点名**，于是这条判据从不上场，存档照旧命中。现在点名了（点的是**当前运行的这一版**），判据生效：存档里没有您这一版 ⇒ 真的联网重拉 ⇒ 页面上就是您刚装上的那版。
  - **横幅多了一道闸**：就算联网也拿不回点名的那一版（比如离线、走了存档兜底），横幅**就不挂**——宁可少说一句，不说错一句。判据是「**记下要宣告的那一版，取回来对上了才画**」，而不是原先的「反正这是自动弹那一次就画」。
  - **顺手补的一处**：您自己点左窄栏切到某一版时，先撤掉「要挂横幅」的待办。原先靠「请求里带没带版本号」间接挡着，改成按「这次取数是不是自动弹那一次」直接判——不补这一处，切版本那条路会跟着漏水（负控测试实测会红）。
  - **说明**：这次治的是**刚升级那一下**的体验；**平常用菜单打开**仍旧按设计读 24 小时内的存档（那是「同一版本不重复拉」那条验收判据要求的，不动）。**实机验收（含这次的横幅）与 `#57.13f` 的七点一并做。**
- **随包插件 `lang-defaults` 1.0.13 → 1.0.14**（关于页 9 个字段名的英文补译）——上一版出厂时漏重打包（改了 `en.json` 没重打 zip），英文界面下「关于 LinkDesk」页这几行显示的是中文原文；本版随包补回。

## v0.1.58（2026-09-13）

- **feat:E6#57.14 「关于 LinkDesk」现在是一个能打开的页面了——版本号、引擎版本、系统版本一处看全，还能一键复制出来贴进问题反馈**（第 5.4 轮｜一个壳渲染的标签页 ＋ 两条命令 ＋ 两处菜单入口 ⇒ 纯新增，0.x 向后兼容走 patch 位）
  - **三个入口，都通到同一个页面**：帮助菜单的「关于 LinkDesk」（在「检查更新…」正下方，两者中间不画线——它们读起来是一件事：「关于本机 / 软件更新」）、设置齿轮菜单里的同名一项、以及命令面板。**同一个软件里只会有一个关于页**（再点一次是切到它，不是再开一个）。
  - **页面上有什么**：最上面是软件标志和名字，下面是八行「名称 → 值」——版本、提交、日期、Electron、Chromium、Node.js、V8、OS。**这些值的唯一来源就是软件自己**（版本号取自运行时，其余取自引擎运行时与系统），页面不自己拼一个字。
  - **「检查更新…」**：和帮助菜单、齿轮菜单、标题栏那颗按钮**是同一个功能**——这里只是它的第四个入口，结果照旧落在状态栏铃铛的通知里（最新 / 有更新 / 失败可重试）。**不为它另做一个结果展示区**：同一个结果两个地方显示，早晚会说出两句不一样的话。
  - **「复制」**：把八行按「名称: 值」逐行写进剪贴板（纯文本），然后铃铛里出一条「已复制」。**复制不出来的时候不撒谎**——万一此刻还没拿到数据，就什么都不做，也不弹「已复制」（弹了就是在说复制好了，其实没复制）。
  - **读不出身份也不崩**：开发期裸跑（没有 `product.json`）时，提交和日期显示一个「—」，其余照常。**取数失败同样不停在加载图上**——页面照常出现，值显示为「—」；下次再打开会重新试一次，不会把一次偶发失败记成「这台机器永远读不出身份」。
- **如实记账（三处与验收图/设计稿对不上的地方，已同步改文档）**
  - **软件标志用的是真标志文件，不是验收图上手画的那个方块**：验收图上那个「圆角蓝方块 + 白方块」是示意图。仓库里的真标志文件自己的注释写着「替换此文件即可更换 logo，无需改代码」——这里要是手画一个方块，那句话对这个页面就变成假话，而且同一个软件里会出现两个不一样的「LinkDesk 标志」（标题栏一个、关于页一个）。所以改用真文件。**全仓库只有一个标志文件、一处规则。**
  - **版本号是纯版本号，不带验收图上那个「(user setup)」后缀**：那串是安装包语境下的完整显示名，这一页要的是版本号本身。
  - **按钮和文字的尺寸有几处取整**：验收图上的间距（22px / 14px / 10px）和字号（16 / 12.5 / 11.5px）过不了本项目的机械门禁，按规则就近取整；颜色也全部改走主题变量（验收图上写死的两个颜色，一个正好等于主题里的强调色、一个正好等于强调色上的文字色）。**这是门禁逼的，不是另选了一版设计。**
- **说明**：验收图上还有一个「加载中」的样子（本次没有画稿，按正式页面同形画了一个骨架，所以数据到位时页面不跳动）。**本页对外的实机验收（含复制到剪贴板、断网/无 product.json 的降级、语言切换）与 `#57.13` 的七点一并放在实机验收批里做**（2026-09-13 用户拍板：`#57.14f` 移进实机批）。开发格内已做的是单元级等价物。
- **作者轴（npm）**：`@linkdesk/contracts` 升到 **0.1.11** 并**已随本版上架**（2026-09-13 用户拍板「攒到第 ⑦ 格一起发」——`#57.13` 发行说明与 `#57.14` 关于页两次载荷类型增量**合成一个号**，中间的 `0.1.10` 只在仓里存在、从未上架）。货架 `latest` 实测 = 0.1.11。`@linkdesk/plugin-sdk` / `@linkdesk/ui` / `create-linkdesk-plugin` 三包本次不动。
...
- **fix:E6#57.12 收尾——「上次更新没装完」这件事，现在下次开机会告诉您了**（第 5.4 轮｜一处主进程归因修复 ＋ 11 句带数字的失败原因第一次能被翻译 ＋ 打包产物可复现 ⇒ 0.x 向后兼容走 patch 位）
  - **您现在能看见的**：上一次更新**没装成**（比如下载完了、您点了重启，但安装程序已经不在了），过去是**一声不吭**——开机后什么提示都没有，顶栏也不会出现按钮，您只会觉得"更新没反应"。现在开机会出一条通知说明**上次未完成**，配一个**「重新下载」**按钮，点一下重新走一遍（先重新查一次，再重新下）。
  - **为什么以前不吭声**：主进程其实**算出了**这件事，但它把结果归成了"下载中断"，而那条归因是给"下载中途断线"用的——它只会由**发起下载的那个人**说出来。开机这件事**没有发起人**，于是这句话永远没人说。现在把它拆成一个独立的归因，谁都能分辨。
  - **英文界面少显示中文的地方又少了一处**：更新失败的原因句里，凡是**带数字的**（如"下载超时——30 秒无数据"、"下载中断——已收 1.2/5 字节"、"安装器写盘失败（磁盘满 / 无权限）——<系统原文>"）以前是把数字**拼进句子**的，这种句子在英文界面下永远只能显示中文。现在改成"模板 + 填数字"，**这 11 句第一次能翻译了**；即使某句还没翻译，里面的数字也会被正确填进去（不会把 `{{seconds}}` 原样漏给您）。
  - **打包过程修了一处「看天吃饭」**：捆绑插件的打包产物以前**取决于打包那台机器的换行符状态**，导致同一份源码重打一次内容指纹就变了，发布前的检查会满屏报红（而源码一个字没改）——那种假红会让人条件反射去乱改版本号。现在打包一律归一，**并且把时间戳钉死**：同样的输入，产出**同样的字节**。
  - 插件 `lang-defaults` **1.0.11 → 1.0.12**（新增上面那 11 句 + 「重新下载」共 12 条英文词条）。⚠️ 该插件的 12 个捆绑包**内容只差换行符**，故**不改插件版本号**——换行符不是内容，版本号留给真变更（发布前的检查已按这条判据放行，并各加一条正反例钉住）。
