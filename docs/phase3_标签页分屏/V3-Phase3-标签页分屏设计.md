...
> 关联：[V3设计方案.md §3](../phase1_架构设计/V3设计方案.md) / [V3开发计划.md](../开发管理/V3开发计划.md)
...
## 1. 为什么在卡片之前做——两层容器打破 V2 面板筒仓

### V2 的痛：面板是封闭的筒仓

V2（WPF 版本）的面板架构是硬编码的——Sensors、Sliders、Keys、Plot、Log 各自独立，布局固定。想同时看两组传感器数据？做不到。想把 Plot 放左边、Sensors 放右边？做不到。Sensors.cs 膨胀到 3570 行，因为所有卡片类型全塞一个面板里——面板和卡片逻辑混在一起，没有边界约束。

### V3 的解法：两层可组合容器

V3 用**两层容器**替代 V2 的单层面板筒仓：
...
```
┌──────────────┬──────────────────────────┐
│  📟 终端      │  📊 心率检测工作台         │  ← 外层：分屏决定"看什么"
│  原始数据流   │  ┌─────┐ ┌─────┐ ┌───┐  │
│              │  │心率 │ │温度 │ │PID│  │  ← 内层：卡片决定"怎么摆"
│ [chip,99]    │  │ 72  │ │36.5 │ │...│  │
│ [temp,36]    │  └─────┘ └─────┘ └───┘  │
├──────────────┼──────────────────────────┤
│  📊 车距检测  │  🎨 OLED 模拟             │
...
拖拽标签页到主区边缘 → 左右或上下分屏。**支持递归嵌套——对标 VS Code 自由布局（3-pane、2x2、左上下+右单等）。** 实现见 [V3-Phase3-补充-递归分屏.md](V3-Phase3-补充-递归分屏.md)。
