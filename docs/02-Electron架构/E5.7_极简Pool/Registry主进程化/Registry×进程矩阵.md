...
3. **FileDecorationRegistry 全链无注册方**：消费端就位（池 FileTreeDecoration 经 `lk.decorations` 代理查询），但全仓零 `registerProvider` 调用 → 壳侧代理目标恒空。功能未铺开，Phase 12 #60 按需补。
4. **CardRegistry + DataDispatch 是遗留空壳**：零写入方（lifecycle 只有 unregisterPluginCards），零消费者——卡片工作台未实现时代的骨架。
...
| **FileDecorationRegistry** | `_providers`（JS 函数） | **零注册方**（全仓无 registerProvider 调用） | 池 FileTreeDecoration → `lk.decorations` → `decorations:getDecoration` 代理 → 壳（恒空） | ⚠️ 消费端就位、注册方缺失。provider 是 JS 函数不可跨进程 → **不迁**（设计稿一致）；注册/消费同池，Phase 12 #60 按需补 |
...
| **CardRegistry** | `_cards` | **零写入方**（lifecycle 仅 unregisterPluginCards） | **零消费者**（DataDispatch 只被 CardRegistry 反引） | ⚠️ 遗留空壳——卡片工作台骨架（Phase 7 时代），候选死代码 |
...
3. **不迁清单维持设计稿**：CommandRegistry/MenuRegistry/KeybindingRegistry/ConfigurationRegistry/ContextKeyService（已有代理+桥，且壳执行逻辑依赖）；FileDecorationRegistry（JS 函数，同进程原则）。
4. **意外发现入其他 Phase**：marketplace 直 import → #56 已有；CardRegistry/DataDispatch 遗留空壳 → 建议登记死代码任务；StatusBar 动态 API 零调用 → E6 面。
