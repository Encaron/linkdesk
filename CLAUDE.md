...
| 🔥 插件测试覆盖层（L11，2026-09-25 立案） | `docs/02-Electron架构/E6_插件生态与发布/插件测试覆盖层/00-整理档案.md`——`E6#144`-`E6#154` 十一格、**5 会话接力**：**地基收敛（#144 把 `vitest.setup.ts` 收进 SDK 一个 subpath 真源，→ SDK 0.1.47）→ 脚手架与 5/18 仓铺开（#145，→ scaffold 0.1.14）→ 文档（#146，→ docs 0.1.31）∥ 三仓并行补测（#147/#148 serial-monitor · #149/#150 file-tree＋editor＋settings · #151/#152 marketplace，均不 bump）→ 覆盖尺 `audit:plugin-tests` ＋ 销账收口（#153/#154，**只报不拦**：⛔ 不进 `npm run check`/CI/`ci-verify.mjs`）**；🔴 **官方 5 仓不 bump**（测试不进 `.linkdesk-plugin` 产物；例外 = 补测发现真 bug ⇒ `fix:` ＋ 目录收录——🟢 **2026-09-25 用户授权：先汇报再改、版本随之变允许**）、**壳 0.2.15 不动**、⛔ **第三方仓 `geme-tihu-bicycle` 不代改**（现存第 6 份 `vitest.setup.ts` 副本保持原样，建议转达作者）；13 只声明式插件仓**有据豁免**（无可测单元）；队列唯一真相源 = 同夹 `交接.md`；🔴 **层内两条硬次序** = SDK 0.1.47 必须先上架 npm 货架（否则 5 仓 lock 装不到新 subpath ⇒ CI 全红）、覆盖尺必须等前四棒收口；⚠️ **一个会话只做一个轮次** |
