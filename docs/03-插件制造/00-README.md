...
| 6 | `06-plugin.json规范.md` | plugin.json 全部字段参考——**注意：这里没有 `readme` / `changelog` 字段**，说明与更新日志以文件为准 |
| 7 | `07-插件间通信.md` | 插件之间怎么传数据——**三通信机制**：事件广播 / 命令调用 / 数据管道（高频推流） |
| 8 | `08-ViewContainer-视图容器API.md` | 如何注册侧栏/面板视图、往别人的容器里加内容、titleActions 声明制 |
| 9 | `09-插件目录规范.md` | 插件目录结构——文件放哪、命名约定 + **`README.md` / `CHANGELOG.md` 的权威判据**（详情页两个页签的数据源） |
...
| 13 | `13-插件开发指南.md` | **🔥 第一次写插件从这里开始**——从 `npm create` 到上架的九步动线（每步含可验证判据 + 该细读哪一篇）。本表其余各篇是参考手册，这篇是路线 |
| 14 | `14-数据管道命令约定.md` | 数据管道命令的命名与注册约定——`<pluginId>.sendBytes` / `onData` / 协议编解码 |
| 15 | `15-多仓开发与本地工作区.md` | 插件工程放哪（容器目录）、仓库名怎么起、**容器绝不建仓**红线、本地目录与插件身份无关 |
| 16 | `16-命名规范.md` | **七种名字一条身份**——id 怎么起/能不能改、显示名怎么改、仓库叫什么、版本号四处同源、分发件名与 tag 不许手改 |
