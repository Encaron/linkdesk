...
export default [
  ...
  // 静态声明三表（LangDef/Protocol/FileAssociation）唯一写入方 = electron/plugin-manifest-loader.ts
          ...
          patterns: [
            {
              ...
              message:
                "🚫 E5.7#52：主进程禁止 import 壳侧注册表模块——跨进程模块实例隔离，主进程拿到的是自己的空实例。LangDef/Protocol/FileAssociation 唯一写入方 = electron/plugin-manifest-loader.ts，读取走 electron/ipc/registry-handlers.ts IPC；其余注册表经 IPC 收取，禁止主进程直 import。type-only import 用 `import type` 即可通过。",
            ...
            },
            {
              ...
              message:
                "🚫 E5.7#52：主进程禁止 import 壳侧注册表服务——跨进程模块实例隔离，主进程拿到的是自己的空实例。FileAssociation 唯一写入方 = electron/plugin-manifest-loader.ts，读取走 electron/ipc/registry-handlers.ts IPC；ViewContainerService 经 IPC 收取，禁止主进程直 import。type-only import 用 `import type` 即可通过。",
              allowTypeImports: true,
            },
          ],
        },
