...
| E3h | **🔥 美化专题**——MenuRenderer 归一化 + TitleBar 扩展 + when 禁用态 + CSS 审查 | 新增 | 归一化收尾 |

> 旧 E3d "Profile与壳完善"（18 任务/1,060 行）太胖——拆为 E3d/E3e/E3f/E3g 四个独立可验证的子任务。E3h 是 #52 完成后发现的渲染归一化缺口。
...
任务 ID 从 `#24` 到 `#82g`，承接 E2 的 `#1`-`#23` + `#12a` + `#12b` + `#13a` + `#13b` + `#17a` + `#19a`-`#19l`（E2d #20-#23 已取消）。E3a-E3j 核心任务全部完成 + 支线 `serial-port-refresh` 7 commits（#82a-#82g）。
...
| 6 | `06-E3f-壳UI收尾.md` | 标题栏暗色化 + ☰ 基础四组 + 齿轮菜单 + 输出面板 + 欢迎页 + Workspace IO + 会话持久化 + 🔥 FileDecorationRegistry + 🔥 SelectBox 归一化 |
| 7 | `07-E3g-API与V2兼容.md` | StatusBarItem + 共享图标 + ☰ 完整版（快捷键/禁用态/插件菜单） + V2 配置导入 |
| 8 | `09-E3h-美化专题.md` | 🔥 MenuRenderer 归一化 + 快捷键归一化 + TitleBar 扩展槽 + when 条件 + CSS 审查 |

## 做完 E3 后的事——不占用编号

```
E4_文件树与编辑器/         ← 第一批消费者插件（E4 编号——最后一批）
...
- 原 P7 设计已吸收到 E3a-E3g + E4 文件树与编辑器
...
> **🏁 E 编号到此为止。** E4 是最后一个编号——文件树 + Monaco 是第一批消费者插件，最后一批有 E 编号的任务（#83-#116）。此后全是插件。
