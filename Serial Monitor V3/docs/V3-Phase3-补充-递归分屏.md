...
V2 的面板是封闭的、硬编码的——Sensors、Sliders、Keys、Plot、Log 各自独立，布局固定。想同时看两组传感器数据？做不到。想把 Plot 放左边、Sensors 放右边？做不到。

**V3 用两层容器打破了这个限制：**

| 层 | 名字 | 机制 | 对标 |
|------|------|------|------|
| **外层** | 标签页 + 递归分屏 | 每个标签页是一个独立容器，可以 N 个同时打开、自由分屏排列 | VS Code 编辑器组 |
| **内层** | 卡片系统 | 每个工作台标签页内部是卡片网格，卡片可以拖拽重排、自由组合 | 桌面上的窗口 |

**两层组合起来 = 用户想怎么摆就怎么摆：**

```
┌──────────────┬──────────────────────────┐
│  📟 终端      │  📊 心率检测工作台         │  ← 外层：分屏决定了"看什么"
│  原始数据流   │  ┌─────┐ ┌─────┐ ┌───┐  │
│              │  │心率 │ │温度 │ │PID│  │  ← 内层：卡片决定了"怎么摆"
│ [chip,99]    │  │ 72  │ │36.5 │ │...│  │
│ [temp,36]    │  └─────┘ └─────┘ └───┘  │
├──────────────┼──────────────────────────┤
│  📊 车距检测  │  🎨 OLED 模拟             │
...
**Phase 3 先把外层容器做好。** Phase 4 卡片系统进场后直接享受这个自由度——任何卡片可以出现在任何工作台标签页里，任何工作台标签页可以和终端/OLED 自由分屏。这就是 V3 相对 V2 的核心架构升级。

> 这个设计的哲学：**分屏自由（Phase 3）+ 卡片自由（Phase 4）= 用户想怎么摆就怎么摆。** Phase 3 的目标就是先把容器造好，等卡片进场。
