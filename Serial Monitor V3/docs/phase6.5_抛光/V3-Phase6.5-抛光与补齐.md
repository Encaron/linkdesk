...
| **6.5b** | 通用 API（2 项） | ~90 | 低 | Phase 5 StatusBarService + i18next |
...
**Phase 6 + 6.5 完成后的完整能力：**

| 维度 | Phase 6 结束 | Phase 6.5 结束 |
|------|------------|--------------|
| 文件操作 | 浏览/打开/搜索/多选/编码/拖拽/关联/图标 | — |
| 文本编辑 | Monaco JSON + schema + 多工作区 | — |
| 通知 | 基础 Toast | + 进度条/过滤/面板/历史 |
| 状态栏 | 静态 manifest | + 运行时动态创建 |
| 图标 | codicon + 文件图标主题 + 产品图标主题 | + 共享图标 |
| 菜单栏 | 基础四组 | + 快捷键提示 + 插件顶级菜单 |
| i18n | 核心翻译 | + 插件自带翻译文件 |
| V2 兼容 | 无 | + V2 配置导入 |
