...
| **Contract required-surface drift is now zero** | D1 `env.get(pluginId)` forwarding, D2 `clipboard.readText` shell completion, D3 `dialog.openFile` shell completion — all three are implemented, with no `?` degradation |
