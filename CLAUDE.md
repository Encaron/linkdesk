...
#     🔴 **判据⑨「token 作用域」（E6#109n-b · 1.24 落）**：自定义属性的定义作用域约束——文档级只有
...
#       ⚠️ 本条是**规则宣告**（1.23 定案），**判据实现归 1.24**；落地前 `check-css-namespace` 尚无此判据
#   ⚠️ **插件侧的另一条腿不在这里**：同一条规则的插件版是 SDK 的 `check-css-namespace` 腿
#     （`@linkdesk/plugin-sdk` 0.1.25 起随包下发，**插件仓 CI 的严格腿判红** —— 在插件工程根 `npm run verify`）；
