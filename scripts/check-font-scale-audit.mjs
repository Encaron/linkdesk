 ...
 *   🔴 **裁决（1.28a 细查后定）：不豁免——判据保持现状，把假话删掉即可。** 三条依据：
 *     ① **CSS 里 `@font-face` 根本没有 `font-size` 这个描述符**——它只吃 `font-family` / `src` / `font-weight` /
 *        `font-style` / `unicode-range` / `size-adjust` 这一族 **font descriptor**，而 `font-size` 是**属性**不是描述符
 *        ⇒ 写在块里就是**无效声明、浏览器直接忽略** ⇒ 判据报它抓的是一处**笔误/死代码**，正是该干的活；
 *        也**不存在「合法写法被误伤」**的场景 ⇒ 没有假红可言。
 *     ② **今天全仓 0 个 `@font-face` 块**（实测 `src/` ＋ `plugins/` 全扫，一个都没有）⇒ 这个豁免是**纯假设**；
 *        为假设开一个跳过口 = 给未来留一个没人要的洞（本系列最贵的纪律就是「别开不需要的口」）。
 *     ③ 那句假话的出处查过了（`git log -S`）：**E5.8#180 立这道门禁那一笔**（`691af9aeb`）的提交消息里
 *        就写着「注释/@font-face/dist/自定义属性放行」——**从落地第一天起就写错了**，不是后来漂移。
 *        ⇒ 若将来真出现「必须豁免」的**实证**（例如某插件把尺寸写进字体块、且引擎真的吃），**先拿实证再来加**。
