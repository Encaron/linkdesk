...
import { RingBuffer } from "../../../core/data/RingBuffer";
...
function createHandlers() {
  ...
  const onSerialData = (payload: string) => {
    if (!portOpenRef.current) return; // 关门则丢弃
    ringBuffer.write({ text: payload, type: "received" });
  };

  const onSerialSystem = (payload: string) => {
    ...
    ringBuffer.write({ text: payload, type: "system" });
  };
...
describe("serial handlers", () => {
  ...
  it("关闭后 serial-data 被拒绝（portOpenRef 门控）", () => {
    ...
    onSerialSystem("---- 已打开串行端口 COM3 ----");
    expect(portOpenRef.current).toBe(true);

    // 关闭
    onSerialSystem("---- 关闭串行端口 COM3 ----");
