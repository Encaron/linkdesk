 ...
 *
 * 策略表 = 壳层窗口策略声明层（mode → 初始布局组装/空窗行为/关窗×语义/[+] 供给）。
 * 新增窗口类型 = WindowMode 加成员 + 策略表加一行——本测试保证每个模式都有策略且声明齐全，
 * 且 main/detached 按架构语义区分（main 全 zone + 欢迎兜底 + 退出语义；detached 子集 + 空窗自灭 + 关闭 tab）。
 */
...
describe("WINDOW_MODE_STRATEGIES 窗口模式策略表", () => {
  it("每个窗口模式都有策略且声明齐全", () => {
    const modes: WindowMode[] = ["main", "detached"];
