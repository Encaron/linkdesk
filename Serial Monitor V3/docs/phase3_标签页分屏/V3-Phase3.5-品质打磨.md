...
- [x] `grep` 硬编码 hex（`#[0-9a-fA-F]{6}`）在 `.tsx/.css` 中返回空（CSS 变量定义和 rgba 除外）
- [x] `grep` 硬编码中文（非注释/非 t() 内）在 `.tsx/.ts` 中返回空
- [x] `grep` `z-index: [0-9]` 在 `.css` 中返回空（全部替换为 `var(--z-*)`）
- [x] 亮色主题：顶栏 ☀ 一键切换，全局跟随
- [x] 英文模式：顶栏 中/EN 一键切换，全局英文化
- [x] Tab 键全程可见焦点（2px `var(--accent)` 环）
- [x] `npx tsc --noEmit` 零错误
- [x] `npx vitest run` 全部通过（109 个，7 文件全通过）
- [x] `npx tauri dev` 正常运行
