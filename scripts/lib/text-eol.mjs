 ...
 *
 * 为什么要有这个文件：同一条规则被**多个脚本共用**——
 *   - 打包侧（原 `scripts/pack-bundled-plugins.mjs`，已随 E6#101 退休；打包实现现在只在
 *     `@linkdesk/plugin-sdk` 的 `pack` 里）：打包时归一，让**产物**与「打包时工作区的行尾」解耦；
 *   - `scripts/check-bundled-version-bump.mjs`：比指纹前归一，让**基线比对**不被行尾污染；
 *   - `scripts/lib/bundled-fingerprint.mjs`（E6#101）：出厂种子指纹——同样先归一（内容 ≠ 行尾）。
 * 各写一份必然漂移（改一处忘一处 ⇒ 一边归一一边不归一 ⇒ 假红假绿都出现），故抽到这里。
 *
