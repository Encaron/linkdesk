...
- VS Code Source Reference — 新增"刻骨铭心"节：刁钻 bug 先 curl VS Code 源码
- User Profile — 新增"硬件直觉"优势：物理思维对软件调试的降维打击
- V3 Pitfalls — B80-B83 已追加
- 插件UI写法规约（`docs/03-插件制造/00-插件开发/插件UI写法规约.md`）——§7：侧栏列表选中用 `onMouseDown`，对标 VS Code Explorer
