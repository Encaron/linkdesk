...
| 🔥 UI 集中供给（L9，2026-09-19 立案） | `docs/02-Electron架构/E6_插件生态与发布/UI集中供给/00-整理档案.md`——`E6#121`-`#127` 七格：@linkdesk/ui 从「编译进插件 bundle」翻转为「池 vendor 单实例供给」（react external 机制推广；插件代码/契约零改动，壳改 UI 全生态跟走）。🔴 **`#122` vendor spike 真机 probe = 全批 go/no-go 闸（红即停批）**；🔴 契约纪律 = 导出面只加不删（#121 先立闸）+ 同版本内行为不变；**版本重锚 = ui 包与壳同号锁步（E6#124 定案：重锚号 0.2.13，锁步断言 = 发布门禁判据⑤；ui 0.3.x 线弃用）**；🟢 发版已授权、插件仓推送仍等用户点头；**皮骨拍板：皮全开放（token 主题域）、骨不开放（壳 chrome 结构永不作为插件能力）**；队列唯一真相源 = 同夹 `交接.md`；默认编排 3 会话（A=#121+#122 · B=#123+#124 · C=#125-#127） |
