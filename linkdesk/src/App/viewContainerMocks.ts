 ...
 * panelCreatePicker（#32）/ panelReveal（#34.5）同 mock 同一壳服务（getViewContainers/getViews/
 * isVisible/setVisible）→ 唯一事实源。测试文件 import 本模块即注册 mock（helper 先于 SUT 求值，
 * vitest 模块依赖序保证）。
 *
...
const { getViewContainersMock, getViewsMock, isVisibleMock, setVisibleMock } = vi.hoisted(() => ({
...
export { getViewContainersMock, getViewsMock, setVisibleMock };
