...
|:--|:--|:--|:--|:--|
| **ContextMenu** | plugin → shell 交互 | 壳 React | ✅ 已 IPC 化（E5#69）——`menu:showContextMenu` | E5.5#9 验证 |
| **DialogService** | 插件代码 | 当前插件调用 `window.linkdesk.dialog.show()` | ✅ 已有 IPC——`dialog:show` | E5.5#10 验证 |
| **Toast** | 插件代码 | 壳级 `window.linkdesk.toast.show()` | ✅ 已有 IPC——`toast:show` | E5.5#12 验证 |
| **SelectBox** | plugins/ 内部 | 插件 React 渲染 | 🔴 限制在插件 WebView 内 | E5.5#13 切 IPC |
| **OverlayPortal** | plugins/ 内部 | ReactDOM.createPortal → document.body | 🔴 portal 到的是插件 WebView 的 body | E5.5#13 切 IPC |
...
## E5.5#13 SelectBox/OverlayPortal IPC 化

- [ ] SelectBox 下拉列表显示在壳 WebView 中（不在插件 WebView 内被截断）
- [ ] 选中回调正确送达插件
- [ ] OverlayPortal → 切换到 OverlayWindow 渲染（E5.5#14-#28 后）
