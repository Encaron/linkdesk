...
> **Status:** final (2026-08-23) — the schema is frozen ([05](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/05-主题数据模型.md) decisions A-F).
...
- **Domain contributor**: write only the domains you own; the rest inherit the shell defaults ([03 Domain Contributors](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/03-域贡献者与混搭.md)) — a pure-color theme = colors only; a glass theme = glass + colors.
- **Style vs. colorway separation**: if you want "1 style base + N colorways" → put `colorways[]` in one recipe ([05 §2](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/05-主题数据模型.md) for the complete schema).
...
|:--|:--|:--|
| radius | six size steps | `--radius-pill`/`--radius-full` form values are not written here (excluded from the scale, [08 §3](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/08-持久化与配置.md)) |
| glass | material keys + floating form | specular/morph = liquid glass's "specular highlight / response" ([02 §2.3](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/02-变量契约.md) material positioning); radius/shadow = floating corner radius / drop shadow (merged into glass after the surface domain was removed; inset is host-derived and border is not a field) |
...
| Wanting to break free of the accent color | Use **your own token** (e.g. `--toggle-on-bg`) instead of reading `--accent` ([04 §6](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/04-两层调节与设置页.md)) |
| Large blur values | `--glass-blur` has a performance ceiling (large-area backdrop-filter is a GPU burden) — the contract will spell out the number ([02 §2.3](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/02-变量契约.md) pending decision) |

## 4. What theme authors get

- Data: `window.linkdesk.theme.listRecipes()` (including colorways/preview colors) — third-party UIs can consume it too ([06 §6](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/06-主题API契约.md) pending decision 3: prefer reusing the general API).
...
- Mix-and-match: your theme automatically shows up among the mix-and-match sources in the settings page according to the domains it contributes (`RecipeMeta.domains`), with **zero extra work for the author** ([10 §2](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/10-混搭设计.md)).

## 5. Migration (existing theme plugins — decision F: zero backward compatibility)

The engine **reads only the new format** (`colorways[]`); the old flat format is no longer read ([05 §5](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/05-主题数据模型.md)). Converting existing theme plugins:
...
- Authoritative schema: [05-Theme Data Model](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/05-主题数据模型.md)
- Authoritative API: [06-Theme API Contract](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/06-主题API契约.md)
- Theme appearance doc set: [Appearance Theming README](https://github.com/Encaron/linkdesk/blob/electron/docs/02-Electron架构/E5.8_归一化基建/外观主题化/README.md)
