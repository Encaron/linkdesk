...
| F3 语言切换即时 | 市场插件订阅壳语言变更事件 → `i18next.changeLanguage` 即时刷新 UI（实现时核对壳事件通道名） | §四 |
...
- **toast 主动作按钮机制**——现有 `notifications.show` 无按钮参数；[重试]/[查看依赖] 动作用**二次 toast 承载**（点击 toast 区域或重新触发），或 E6 后续加 NotificationHandle 按钮字段——实现阶段决定，若加则走设计前置。
