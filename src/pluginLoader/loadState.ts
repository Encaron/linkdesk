...
const ALLOWED: Record<LoadState, LoadState[]> = {
  ...
  loading:   ["active", "failed", "unloading"],
  active:    ["unloading"],
  unloading: ["disposed"],
...
function transition(pluginId: string, next: LoadState, failureReason?: string): void {
  ...
  _states.set(pluginId, { state: next, failureReason: failureReason ?? undefined });
}
