...
2. **记忆索引：`C:\Users\fengy\.claude\projects\e--linkdesk\memory\MEMORY.md`**——**🔥 级索引（2026-09-17 批次 7 起瘦身：只列高频判据与必读；其余 90+ 份记忆文件仍在该目录、本体一个没删，`ls` ＋ 全文 grep 可达）**。先读其「🔥🔥🔥 新 AI 进场——先读这些」一节（AI 工作法正典 / 提交前机械自检 / 硬约束 / 十条戒律…），其余按需 grep 对应文件。
...
| 要落盘的东西 | 写到哪 | 规矩 |
|------|------|------|
| 用户拍板 / 教训 / 工作法 | 记忆库新建条目（frontmatter `name`/`description`/`type`，`description` 写**触发条件**非摘要）＋ **够 🔥 级才**在 `MEMORY.md` 加行（次级条目可两文件合一行） | 格式与自检照 [docs/06-记忆系统整理/02-索引重写规范.md](./docs/06-记忆系统整理/02-索引重写规范.md)（**行数 ≤50 · 断链 = 0**；孤儿只报不判死——沉底是设计，批次 7 起「孤儿 = 0」作废） |
