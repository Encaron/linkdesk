 ...
 *
 * ── 🔴 分级：本腿**从落地起就是红**（1.49 复核时的如实订正）──
 * 1.32 的注释原写「本格只判黄 …… **1.49 才收紧为红**」，但实现里**三条判据的裁决一律进 `violations`**
 * （本文件没有 `advisories` / `yellow` 分栏）⇒ 在**插件仓 CI**（`ci-verify.mjs` 把非 ADVISORY 腿的
 * `violations` 计入严格腿）**一直是红**；作者本地仍不被拦（`bin lint` 的退出码只看 eslint severity 2
 * ——见 `src/bin.ts`）⇒ 「本地黄、CI 红」这个双轨形态与另几条腿一致，只是注释里的时间点写错了。
 * 1.49 保持现状（「收紧为红」对本腿 = 已达成），**只订正注释，一个判据都不动**。
 *
