...
>
> **当前进度：主线 15 步 ✅ 全部完成 + B80-B83 ✅ 修复。下一步 → 第二批（G 类机械修复，步 16-22）。剩余 7 + 11 后续 = 18 步。**
...
#### 步 12：E4 — 接收模式从会话读取　✅ 已完成（`b64fe5c`）

**做什么：** `_receiveMode` 模块级变量 → per-instance `useRef`——每个 TerminalView 实例独立读自己的 `receiveMode`，不共享。

**改什么：** `plugins/terminal/index.tsx` — +6/−6 行
...
#### 步 15：B3 — F5 刷新后会话自动恢复　✅ 已完成（`6f1cf1c`）

**做什么：** 终端插件 mount 时检查——"有没有对应我这个标签页 ID 的会话？"没有就自动创建一个。

**改什么：** `plugins/terminal/useTerminalSessions.ts` — `useSession` hook +18 行。首次 mount 时 `didAutoCreate` ref 守卫——只在组件生命周期首次挂载时检查一次，keep-alive 期间不重复创建。
