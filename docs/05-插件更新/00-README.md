...
| 串口监视器 | 串口收发界面（`plugins/serial-monitor`，随包 bundled） | 待拍板候选：右键菜单项双注册重复（接收区「复制/全选/清空」+ 药丸「编辑/删除」各两次）；⚠️ 与 [Serial-Simulator](Serial-Simulator/README.md) 不是一回事 | [串口监视器/](串口监视器/00-README.md) |
