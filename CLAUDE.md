...
| 🔥 插件兼容机械化（L8，2026-09-18 立项） | `docs/02-Electron架构/E6_插件生态与发布/插件兼容机械化/00-整理档案.md`——`E6#114`-`#119` 六格（悬空名核验 / 只加不删门禁 / 退役登记 / 兼容读数 / 市场显示状态（含 HTML 设计图）/ 判据铺开）＋ **第 7 格 `E6#120`**（通用悬停提示卡＝把那张卡抽成壳的共享件 `HintCard` ＋ `@linkdesk/ui` 导出；**本层最后一棒、出全批收口报告**，详案 = 同夹 `08-任务-通用悬停提示卡.md`）；**队列唯一真相源 = 同夹 `交接.md` 顶部剩余任务队列**（一 AI 一格）；🔴 **用户面文案只许五个词**（正常/兼容/部分不适配/不适配/—，禁词表与三套词表见总纲 §〇d，格 5 必须照办）；生命周期阶段原话与四件机械件现状见 memory `plugin-lifecycle-three-tier` |
