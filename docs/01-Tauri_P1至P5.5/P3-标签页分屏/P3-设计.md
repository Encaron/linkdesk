...
> 关联：[P1-设计方案.md §3](../P1-架构设计/P1-设计方案.md) / [V3开发计划.md](../../开发管理/V3开发计划.md)
...
拖拽标签页到主区边缘 → 左右或上下分屏。**支持递归嵌套——对标 VS Code 自由布局（3-pane、2x2、左上下+右单等）。** 实现见 [P3-补充-递归分屏.md](P3-补充-递归分屏.md)。
...
**为什么 📟/⚙ 保留隐式创建，📊 不保留：**
- 📟 终端是默认视图——启动时必须有一个，关闭最后一个时必须补一个（终端保底）。隐式创建和保底逻辑一致。
