...
> - [P4-欢迎页.md](P4-欢迎页.md) — 欢迎页数据模型 + 状态流转 + UI + 边界情况
> - [P4-终端插件化.md](P4-终端插件化.md) — TerminalView 迁移：依赖分析 + 5 步迁移 + 风险表
> - 数据迁移 — 已合入本文 §14（原独立文档 `V3-Phase4-数据迁移.md`）
> - 测试策略 — 已合入本文 §13（原独立文档 `V3-Phase4-测试策略.md`）
...
| `tabBehavior` | | 标签页行为声明 `{ singleton?: boolean, isFallback?: boolean, confirmOnClose?: string }`。核心读此字段决定去重/保底/关闭确认，不硬编码 switch pluginId。见本文 §12 设计评审关键决策 |
...
- docs `../P3-标签页分屏/P3-设计.md` — Phase 3 设计
...
**旧测试保护：** Phase 4 不允许 91 个旧测试退化。useTabManager 测试中的 `type` 改为 `pluginId`+`tabBehavior`，splitTree/串口/拖拽测试零改动。

**新测试：** loader 测试（8 条——空目录/有效插件/缺字段/JSON错误/同ID版本优先/core标记/构建产物/tabBehavior）+ lifecycle 测试（5 条——启用禁用切换/卸载移到.disabled/重装/不可卸载/json即时生效）+ 欢迎页保底测试（5 条）。

**手动验证 9 项：** CM6渲染/Monaco发送/串口收发/卸载不崩/重装恢复/删目录不崩/主题跟随/语言跟随/分屏各自独立。
...
Phase 3.5→4 的 prefs.json 变化：新增 `locale`/`recentViews`/`enabledPlugins` 字段；旧 terminal 标签页自动补 `pluginId: "terminal"`；总 tab 数为 0 时创建 welcome；终端插件被卸载时显示占位 UI + [安装] 按钮。

**兼容性：** Phase 3.5 能读 Phase 4 的 prefs.json（welcome tab 走 default case 空白页，无害）；新增字段被忽略不报错。

---

## 15. 通知系统（原独立文档摘录）

对标 VS Code Notifications 系统。Toast 右下角堆叠，入场动画 300ms，hover 显露关闭按钮，双击展开详情（source+buttons）。

**数据模型：** `{ severity, source, icon, actions[], ttl }`。actions 分 primary（主按钮）和 secondary。

**生命周期通知：** 安装→通知 + [立即重启]；重装→即时生效无重启；卸载→通知 + [撤销]；禁用→通知 + [撤销]。不自动 reload，不自动跳转。

**Vite 关键修复：** `vite.config.ts` 中 `server.watch.ignored` 增加 `**/plugins/**`，Rust 操作插件目录时 Vite 不触发 reload。

**不做（后续阶段）：** Don't show again / 通知进度条 / Notification Center 面板 / 来源过滤 / 通知归类。
