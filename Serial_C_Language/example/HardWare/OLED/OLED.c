...
{
	HAL_GPIO_WritePin(OLED_SCL_GPIO, OLED_SCL_PIN,x);
}
...
{
	HAL_GPIO_WritePin(OLED_SDA_GPIO, OLED_SDA_PIN,x);
}
...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏模式*/
//    RCC_APB2PeriphClockCmd(RCC_APB2Periph_OLED_SCL_GPIO, ENABLE);
//
...
void OLED_DrawEllipse(int16_t X, int16_t Y, uint8_t A, uint8_t B, uint8_t IsFilled)
{
	...
	while (b * b * (x + 1) < a * a * (y - 0.5))
	{
		...
		if (IsFilled)	//指定椭圆填充
		{
			/* Region 2 (陡坡段)：填充 X-x 到 X+x 全宽（修复：原只填±x两条竖线，中间空缺）*/
			int16_t k;
			for (j = -y; j < y; j ++)
			{
				for (k = -x; k <= x; k ++)
				{
					OLED_DrawPoint(X + k, Y + j);
				}
			}
		}

		}  /* 关闭 Region 1 while 循环（修复：原缺失此括号，导致函数无法关闭）*/
		/*画椭圆两侧部分*/
...
void OLED_ClearLine(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1)
{
...
void OLED_DrawRoundedRect(int16_t X, int16_t Y, uint8_t Width, uint8_t Height,
                          uint8_t Radius, uint8_t IsFilled)
{
    uint8_t r = Radius;
    ...
    {
        /* 填充中央矩形区域 */
        OLED_DrawRectangle(X + r, Y, Width - 2 * r, Height, OLED_FILLED);

        if (r > 0)
        {
            /* 填充左右侧矩形条 */
            OLED_DrawRectangle(X, Y + r, r, Height - 2 * r, OLED_FILLED);
            OLED_DrawRectangle(right - r + 1, Y + r, r, Height - 2 * r, OLED_FILLED);

            /* 填充四个圆角（扇形） */
            OLED_DrawArc(X + r, Y + r, r, 180, -90, OLED_FILLED);               // 左上
            OLED_DrawArc(right - r, Y + r, r, -90, 0, OLED_FILLED);             // 右上
            OLED_DrawArc(right - r, bottom - r, r, 0, 90, OLED_FILLED);         // 右下
            OLED_DrawArc(X + r, bottom - r, r, 90, 180, OLED_FILLED);           // 左下
        }
    }
    else
    {
        /* 画四条直边 */
        OLED_DrawLine(X + r, Y, right - r, Y);                       // 顶边
        OLED_DrawLine(X + r, bottom, right - r, bottom);             // 底边
        OLED_DrawLine(X, Y + r, X, bottom - r);                      // 左边
        OLED_DrawLine(right, Y + r, right, bottom - r);              // 右边

        if (r > 0)
        {
            /* 画四个圆角 */
            OLED_DrawArc(X + r, Y + r, r, 180, -90, OLED_UNFILLED);             // 左上
            OLED_DrawArc(right - r, Y + r, r, -90, 0, OLED_UNFILLED);           // 右上
            OLED_DrawArc(right - r, bottom - r, r, 0, 90, OLED_UNFILLED);       // 右下
            OLED_DrawArc(X + r, bottom - r, r, 90, 180, OLED_UNFILLED);         // 左下
        }
    }
}

void OLED_DrawRotatedEllipse(int16_t cx, int16_t cy, uint8_t a, uint8_t b,
                             int16_t angle, uint8_t IsFilled)
{
    ...
    if (IsFilled) {
        /* 扫描线填充：逐行检查像素是否在旋转椭圆内 */
        int16_t rmax = (a > b ? a : b) + 1;
        for (j = cy - rmax; j <= cy + rmax; j++) {
            for (i = cx - rmax; i <= cx + rmax; i++) {
        ...
        for (int t = 0; t <= 360; t += 5) {
            ...
            if (!first) OLED_DrawLine(prev_x, prev_y, px, py);
            prev_x = px; prev_y = py; first = 0;
        }
    }
}

void OLED_DrawRotatedRect(int16_t X, int16_t Y, uint8_t Width, uint8_t Height,
                          int16_t Angle, uint8_t IsFilled)
{
    float rad = Angle * 3.14159f / 180.0f;
    float cos_a = (float)cos(rad), sin_a = (float)sin(rad);
    float cx = X + Width / 2.0f, cy = Y + Height / 2.0f;

    float corners[4][2] = {{X, Y}, {X+Width-1, Y}, {X+Width-1, Y+Height-1}, {X, Y+Height-1}};
    int16_t rx[4], ry[4];
    for (int i = 0; i < 4; i++) {
    ...
    if (IsFilled) {
        OLED_DrawTriangle(rx[0], ry[0], rx[1], ry[1], rx[2], ry[2], OLED_FILLED);
        OLED_DrawTriangle(rx[0], ry[0], rx[2], ry[2], rx[3], ry[3], OLED_FILLED);
    } else {
        OLED_DrawLine(rx[0], ry[0], rx[1], ry[1]);
        OLED_DrawLine(rx[1], ry[1], rx[2], ry[2]);
        OLED_DrawLine(rx[2], ry[2], rx[3], ry[3]);
        OLED_DrawLine(rx[3], ry[3], rx[0], ry[0]);
    }
}
/*********************功能函数*/
