...
**Phase 3 先把外层容器做好。** Phase 5 卡片系统进场后直接享受这个自由度——任何卡片可以出现在任何工作台标签页里，任何工作台标签页可以和终端/OLED 自由分屏。这就是 V3 相对 V2 的核心架构升级。

> 这个设计的哲学：**分屏自由（Phase 3）+ 卡片自由（Phase 5）= 用户想怎么摆就怎么摆。** Phase 3 的目标就是先把容器造好，等卡片进场。
