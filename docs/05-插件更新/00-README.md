...
- **开发工作流（2026-09-14 起为现行规矩，E6 第 7 层判据先立）**：插件**源码真相源 = 插件自己的 GitHub 仓库**，壳仓只留构建产物（`bundled-plugins/` 种子快照）与文档档案（判据五条 → [`../03-插件制造/09-插件目录规范.md` §源码位置与分发物的关系](../03-插件制造/09-插件目录规范.md)）。单个插件的新版本在仓外独立工程目录开发（布局 → [../03-插件制造/15-多仓开发与本地工作区.md](../03-插件制造/15-多仓开发与本地工作区.md)），直接装到安装版上实测，不必回本仓库全量构建。每只插件单独建仓、单独写文档；本目录只留「这个插件想往哪更新」的档案，不承担开发目录。（源码物理搬迁 = E6 第 7.2 轮，进行中。）
