 ...
 * 换算后 registry / IPC / spawn 全程持绝对路径：lsp-handlers.resolveLspArg 无需知道插件目录
 * （args 已是绝对路径，命中其既有 absolute 分支原样返回）——🔴 spawn 路径零改动。
 *
 * 语义：只改「路径式」arg（含 / 或 \ 分隔符 / 绝对路径 / 脚本扩展名）——flag 类（"--stdio"）
 * 原样保留。与 scripts/check-lsp-deps.mjs 的 isPathLike 同判定，双处写清同一不变式。
 *
 * 契约：plugin.json 的 lsp.args 相对路径，基准 = 该 langDef 所属插件的根目录（文档化于
 * plugin.schema.json + 插件格式文档）。python 插件即示例：args 里 "node_modules/pyright/dist/
 * pyright-langserver.js" → <插件根>/node_modules/pyright/dist/pyright-langserver.js，随插件自带
